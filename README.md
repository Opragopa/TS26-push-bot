...
После нажатия `Отправить в таблицу` бот перечитывает записанную строку. Если запись сработала, в сообщении будет лист, `gid`, номер строки и прямая ссылка на диапазон `A:E`. Если строка не совпала, бот пришлет ошибку с ожидаемыми и прочитанными значениями.
