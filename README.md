...
2. Сделайте redeploy. В логах должна быть версия `2026-07-22.9` или новее.
3. Админом отправьте боту `/debug`.
4. Нажмите `Превью формы`. Это покажет весь пользовательский сценарий без записи в Google Sheet.
5. Для настоящего теста используйте Telegram-аккаунт, которого нет в `TELEGRAM_CHAT_ID`, `TELEGRAM_CHAT_IDS`, `TELEGRAM_ADMIN_CHAT_IDS` и дополнительных получателях `sheets.json`.
6. С этого аккаунта отправьте `/start`, заполните имя и должность, на финальном экране нажмите `Отправить в таблицу`.
7. Проверьте лист `Моушен`: строка должна появиться или обновиться начиная с `280`, а в колонке `E` должна быть пометка `<-- добавлено через ТГ бота`.
